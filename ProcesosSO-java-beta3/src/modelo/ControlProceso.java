...
public class ControlProceso {
    ...
    /*------------------------------------------------------------------------------------------*/
    public void ejecutarTodo() throws InterruptedException {
        ...
        while (!cola_terminadoPrioridaMedia.isEmpty() && stop != true) {
            
            terminado = false;           
            Proceso proceso = cola_terminadoPrioridaMedia.poll();
             ventana.actualizarInformacion(proceso);
            //Ejecuta un proceso de la clase proceso            
            if (proceso.getTamanio_actual() <= 0) {
                ventana.listoToEjecucion();
        ...
        while (!cola_terminadoPrioridadBaja.isEmpty() && stop != true) {
            
            terminado = false;           
            Proceso proceso = cola_terminadoPrioridadBaja.poll();
             ventana.actualizarInformacion(proceso);
            //Ejecuta un proceso de la clase proceso            
            if (proceso.getTamanio_actual() <= 0) {
                ventana.listoToEjecucion();
...
/*-------------------------------------------------------------------------------------------*/
    ...
    public void ejecutar() throws InterruptedException {
        ...
        while(!cola_listoPrioridadAlta.isEmpty()){
            ejecutarPrioridadAlta();
        }
        while(cola_listoPrioridadAlta.isEmpty() && !cola_listoPrioridaMedia.isEmpty()){
            ejecutarPrioridadMedia();
        }
         
        while(cola_listoPrioridadAlta.isEmpty() && cola_listoPrioridaMedia.isEmpty() && !cola_listoPrioridadBaja.isEmpty()){
